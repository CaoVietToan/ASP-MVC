...
namespace SV20T1020595.DataLayers.SQLSever
{
        ...
        public int Add(Supplier data)
        {
            ...
            using (var connection = OpenConnection())
            {
                var sql = @"if exists(select * from Suppliers where Email = @Email)
                                ...
                                begin
                                    insert into Suppliers(SupplierName, ContactName, Provice, Address, Phone, Email,NgayThanhLap)
                                    values(@SupplierName, @ContactName, @Provice, @Address, @Phone, @Email,@NgayThanhLap);
                                    select @@identity;
                                end
                           ";
                var parameters = new
